const translation = {
  ...
  model: {
    params: {
      temperature: '温度',
      temperatureTip:
        '控制随机性: 较低的值会使输出更加集中和确定性，更高的值会引入更多的随机性，使输出更加多样化。',
      top_p: '核采样',
      top_pTip:
        '通过核采样控制多样性: 0.5 表示只考虑概率质量前 50% 的标记。',
      presence_penalty: '存在惩罚',
      presence_penaltyTip:
        '根据新标记是否已经出现在现有文本中来对其进行惩罚。\n增加了模型讨论新主题的可能性。',
      frequency_penalty: '频率惩罚',
      frequency_penaltyTip:
        '根据新标记在现有文本中的频率来对其进行惩罚。\n降低了模型逐字重复相同内容的可能性。',
      max_tokens: '最大令牌数',
      max_tokensTip:
        '用于限制回复的最大长度（以令牌为单位）。\n较大的值可能会限制提示词、聊天记录和知识库的剩余空间。\n建议设置在三分之二以下\ngpt-4-1106-preview、gpt-4-vision-preview 最大令牌数（输入 128k 输出 4k）',
      maxTokenSettingTip: '您的最大令牌数设置较高，可能会限制提示词、查询和数据的空间。建议设置在 2/3 以下。',
      setToCurrentModelMaxTokenTip: '最大令牌数已更新为当前模型的最大令牌数的 80%，为 {{maxToken}}。',
      stop_sequences: '停止序列',
      stop_sequencesTip: '最多四个序列，API 将在这些序列处停止生成更多标记。返回的文本将不包含停止序列。',
    ...
    addMoreModel: '前往设置添加更多模型',
    settingsLink: '模型供应商设置',
    capabilities: '多模态能力',
  },
  menus: {
    status: '测试版',
    ...
    datasets: '知识库',
    datasetsTips: '即将推出：导入您自己的文本数据或通过 Webhook 实时写入数据以增强 LLM 上下文。',
    newApp: '新建应用',
    newDataset: '创建知识库',
    tools: '工具',
    training: '训练',
  ...
  account: {
    ...
    langGeniusAccount: 'X-Stuidio 账号',
    langGeniusAccountTip: '您的 X-Stuidio 账号和相关的用户数据。',
    editName: '编辑名字',
  ...
  members: {
    ...
    invitationSentTip: '邀请已发送，对方登录 X-Stuidio 后即可访问你的团队数据。',
    invitationLink: '邀请链接',
  ...
  provider: {
    ...
    openaiHosted: {
      ...
      onTrial: '体验',
      exhausted: '超出限额',
      desc: '托管 OpenAI 由 X-Stuidio 提供的托管 OpenAI 服务，你可以使用 GPT-3.5 等模型，在体验额度消耗完毕前你需要设置其它模型供应商。',
      callTimes: '调用次数',
  ...
  about: {
    changeLog: '更新日志',
    updateNow: '现在更新',
    nowAvailable: 'X-Stuidio {{version}} 现已可用。',
    latestAvailable: 'X-Stuidio {{version}} 已是最新版本。',
  },
  appMenus: {
    overview: '监测',
    promptEng: '编排',
